...
select mnr as MaschinenNr, name as Name, year(ansch_datum) as Anschaffungsjahr
from maschine 
where year(ansch_datum) < 2000
order by year(ansch_datum) DESC;
...
where a.name = 'Verwaltung' or a.name = 'Projektierung'
order by a.name, p.name, p.vorname ASC;
...
select name as Name, vorname as Vorname, case when p_betrag is null then 1 else p_betrag end as Praemienanzahl
from personal
left join praemie using(pnr)
order by Praemienanzahl desc, name asc, vorname asc; 
